...
// Unit tests (TestPreviewRedactions_FileTooLarge, TestRecordCustody_*,
// TestFinalizeFromDraft_NonPGRepository) and their helper types (errorCustody,
// nonPGRepo) are defined in redaction_coverage_unit_test.go so they run under
// the default unit-test build and are not gated by the `integration` tag.
...
// Unit tests for ApplyRedactions/PreviewRedactions PDF and error-logging
// paths are in redaction_coverage_unit_test.go.

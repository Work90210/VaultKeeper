...
export default async function CaseDetailPage({
  ...
  const [caseRes, evidenceRes, rolesRes, witnessRes, disclosureRes] = await Promise.all([
    authenticatedFetch<CaseData>(`/api/cases/${params.id}`),
    authenticatedFetch<EvidenceItem[]>(`/api/cases/${params.id}/evidence?current_only=true`),
    authenticatedFetch<CaseRole[]>(`/api/cases/${params.id}/roles`),
    authenticatedFetch<Witness[]>(`/api/cases/${params.id}/witnesses`),
    authenticatedFetch<Disclosure[]>(`/api/cases/${params.id}/disclosures`),
  ]);
  ...
  const witnesses = witnessRes.data || [];
  const disclosuresList = disclosureRes.data || [];
  const isProsecutor = roles.some(
